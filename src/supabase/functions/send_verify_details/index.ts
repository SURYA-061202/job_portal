...
const client = new SMTPClient({
  connection: { hostname: HOSTNAME, port: PORT, tls: TLS, auth: { username: USER, password: PASS } },
  ...(TLS
    ? {}
    : { debug: { noStartTLS: true, allowUnsecure: true } }),
});
...
serve(async (req) => {
  ...
  try {
    ...
    if (!candidate?.email) {
      return new Response(JSON.stringify({ success: false, error: "Candidate email missing" }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    ...
    await client.send({ from: MAIL_FROM, to: candidate.email, subject: "Verify Your Details – Indian Infra", content: "Please view this email in HTML capable client.", html });
