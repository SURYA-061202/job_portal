                        ...
                        <nav className="hidden md:flex items-center gap-8">
                            {navLinks.map((link) => (
                                <Link
                                    key={link.to}
                                    to={link.to}
                                    className={`flex items-center gap-2 text-sm font-semibold transition-all relative py-2 group ${isActive(link.to) ? 'text-orange-600' : 'text-gray-500 hover:text-orange-600'
                                        }`}
                                >
                                    <link.icon className={`w-4 h-4 transition-transform duration-300 ${isActive(link.to) ? 'scale-110' : 'group-hover:scale-110'}`} />
                                    <span>{link.label}</span>
                                    <div className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-orange-500 to-primary-600 transition-all duration-300 origin-left ${isActive(link.to) ? 'scale-x-100 opacity-100' : 'scale-x-0 opacity-0 group-hover:scale-x-100 group-hover:opacity-70'
                                        }`} />
                                </Link>
                            ))}
                        </nav>
                ...
                <div className="px-6 py-4 flex flex-col gap-2">
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={`flex items-center gap-4 p-4 rounded-xl transition-all ${isActive(link.to)
                    ...
                    <Link
                        to="/profile"
                        onClick={() => setIsMenuOpen(false)}
                        className={`flex items-center gap-4 p-4 rounded-xl transition-all ${isActive('/profile')
                                ? 'bg-orange-50 text-orange-600 font-bold'
                                : 'text-gray-600 hover:bg-gray-50'
                            }`}
                    >
